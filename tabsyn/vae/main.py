...
def main(args):
    ...
    for epoch in range(num_epochs):
        ...
        model.eval()
        with torch.no_grad():
            X_test_num = X_test_num[:256]
            X_test_cat = X_test_cat[:256]
    ...
    with torch.no_grad():
        ...
        with torch.no_grad(): 
            ...
            train_z_list =[]
            for i in range(0, X_train_num.shape[0], chunk_size):
                num_chunk = X_train_num[i : i + chunk_size]
                cat_chunk = X_train_cat[i : i + chunk_size]
                z_chunk = pre_encoder(num_chunk, cat_chunk).detach().cpu().numpy()
                train_z_list.append(z_chunk)
